    ...
    run: function(creep) {
        ...
        if(creep.memory.status == true){
            var work_targets = creep.room.find(FIND_STRUCTURES, {
                    filter: (structure) => {
                        return (structure.structureType == STRUCTURE_EXTENSION || structure.structureType == STRUCTURE_SPAWN || structure.structureType == STRUCTURE_TOWER || structure.structureType == StructureStorage) && structure.store.getFreeCapacity(RESOURCE_ENERGY) > 0;
                    }
            });
            //如果没有充能目标
